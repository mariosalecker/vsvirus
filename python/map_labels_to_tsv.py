...
path_prodigy_labeled="data/dataset_labeled_prodigy_small.json"
...
with open(path_prodigy_labeled) as f:
    for line in f:
        ...
        text=data["text"]# compliance01_p - 00
        m = re.search(r'(.*)_p-(\d+)', text)

        doc_name=m.group(1)
        doc_page=int(m.group(2))
        answer=data["answer"]
        if "accept" not in answer: continue
...
​
documents_all = glob.glob("./data/kfw/**/*.tsv")
...
result.to_csv("extracted_values.csv")
result.to_excel("extracted_values.xlsx")
print(result)
